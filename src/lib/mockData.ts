...
// --- Statistiques dérivées, calculées à partir des données ci-dessus ---
...
export function getDashboardStats() {
  return {
    ...
    ticketsEnCours: tickets.filter((t) => t.statut === "En cours").length,
    ticketsResolus: tickets.filter((t) => t.statut === "Résolu").length,
...
export function getTicketsParMois() {
  const moisLabels = [
