...
import { getDashboardStats, getTicketsParMois, tickets } from "@/lib/mockData";
...
export default function DashboardPage() {
  ...
  const ticketsParMois = getTicketsParMois();
  const derniersTickets = [...tickets].slice(0, 3);
  ...
  return (
    <div className="p-6">
      ...
      {/* 4 CARTES DE STATISTIQUES (calculées à partir des données réelles) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white shadow rounded-lg p-6 border-l-4 border-orange-500 hover:shadow-lg transition-shadow">
          ...
          <p className="text-xs text-blue-600 mt-2">
            {Math.round((stats.abonnementsActifs / stats.clients) * 100)}% des clients
          </p>
        </div>
      </div>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4 h-32 overflow-y-auto text-sm">
            ...
            <p className="text-gray-600 mt-2">
              <span className="font-medium text-gray-800">👤 Client :</span> Mon internet
              ne fonctionne plus
            </p>
            <p className="text-gray-600 mt-2">
              <span className="font-medium text-blue-600">🤖 IA :</span> Avez-vous essayé
              de redémarrer votre routeur ?
            </p>
          </div>
          <Link
            href="/assistant"
